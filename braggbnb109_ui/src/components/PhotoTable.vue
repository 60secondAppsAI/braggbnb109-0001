        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalPhotos = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPhotos">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Photo</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PhotoId" type="text" placeholder="Enter PhotoId" v-model="photoToAdd.photoId"></base-input>
  <base-input label="Url" type="text" placeholder="Enter Url" v-model="photoToAdd.url"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="photoToAdd.description"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="photos" :row-key="record => record.PhotoId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PhotoPictureView :photos="photos" />
          </div>
...
import PhotoPictureView from './PhotoPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'photoId',
          visible: true,
          scopedSlots: { customRender: 'photoId' },
          sorter: true
          //sorter: (a, b) => a.photoId - b.photoId,
      ...
      sortBy: 'photoId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPhotosTable() {
      ...
      for (let i = 0; i < this.photos.length; i++) {
        photosTableData.push({
          ...
          year: this.photos[i].year,
          date: this.photos[i].date,
          competitionId: this.photos[i].competitionId,
          photoId: this.photos[i].photoId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-photos',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPhotosChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalPhotos = false;

      const currentDate = new Date().getTime();
      this.photoToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.photoToAdd);
      console.log(jsonData);
      
      const res = await PhotoService.addPhoto(jsonData);
