        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalCancellations = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCancellations">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Cancellation</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="CancellationId" type="text" placeholder="Enter CancellationId" v-model="cancellationToAdd.cancellationId"></base-input>
  <base-input label="CancellationDate" type="text" placeholder="Enter CancellationDate" v-model="cancellationToAdd.cancellationDate"></base-input>
  <base-input label="Reason" type="text" placeholder="Enter Reason" v-model="cancellationToAdd.reason"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="cancellations" :row-key="record => record.CancellationId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <CancellationPictureView :cancellations="cancellations" />
          </div>
...
import CancellationPictureView from './CancellationPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    CancellationPictureView
  },

  data() {
    return {
      modalCancellations: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'cancellationId',
          visible: true,
          scopedSlots: { customRender: 'cancellationId' },
          sorter: true
          //sorter: (a, b) => a.cancellationId - b.cancellationId,
          //sorter: (a, b) => a.cancellationId.localeCompare(b.cancellationId),
        },
        {
          title: 'Cancellation Date',
		dataIndex: 'cancellationDate',
          visible: true,
          scopedSlots: { customRender: 'cancellationDate' },
          sorter: true
          //sorter: (a, b) => a.cancellationDate - b.cancellationDate,
          //sorter: (a, b) => a.cancellationDate.localeCompare(b.cancellationDate),
        },
        {
          title: 'Reason',
		dataIndex: 'reason',
      ...
      sortBy: 'cancellationId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCancellationsTable() {
      ...
      let cancellationsTableData = [];
      for (let i = 0; i < this.cancellations.length; i++) {
        cancellationsTableData.push({
          id: i,
          cancellationId: this.cancellations[i].cancellationId,
          year: this.cancellations[i].year,
          date: this.cancellations[i].date,
          competitionId: this.cancellations[i].competitionId,
          cancellationId: this.cancellations[i].cancellationId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-cancellations',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCancellationsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalCancellations = false;

      const currentDate = new Date().getTime();
      this.cancellationToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.cancellationToAdd);
      console.log(jsonData);
      
      const res = await CancellationService.addCancellation(jsonData);
