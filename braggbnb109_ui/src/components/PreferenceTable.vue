        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalPreferences = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPreferences">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Preference</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PreferenceId" type="text" placeholder="Enter PreferenceId" v-model="preferenceToAdd.preferenceId"></base-input>
  <base-input label="PreferenceType" type="text" placeholder="Enter PreferenceType" v-model="preferenceToAdd.preferenceType"></base-input>
  <base-input label="Value" type="text" placeholder="Enter Value" v-model="preferenceToAdd.value"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="preferences" :row-key="record => record.PreferenceId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PreferencePictureView :preferences="preferences" />
          </div>
...
import PreferencePictureView from './PreferencePictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        {
          title: 'Preference Id',
		dataIndex: 'preferenceId',
          visible: true,
          scopedSlots: { customRender: 'preferenceId' },
          sorter: true
          //sorter: (a, b) => a.preferenceId - b.preferenceId,
          //sorter: (a, b) => a.preferenceId.localeCompare(b.preferenceId),
        },
        {
          title: 'Preference Type',
		dataIndex: 'preferenceType',
          visible: true,
          scopedSlots: { customRender: 'preferenceType' },
          sorter: true
          //sorter: (a, b) => a.preferenceType - b.preferenceType,
          //sorter: (a, b) => a.preferenceType.localeCompare(b.preferenceType),
        },
        {
          title: 'Value',
		dataIndex: 'value',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} preferences`,
      },
      ...
      sortBy: 'preferenceId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPreferencesTable() {
      ...
      let preferencesTableData = [];
      for (let i = 0; i < this.preferences.length; i++) {
        preferencesTableData.push({
          id: i,
          preferenceId: this.preferences[i].preferenceId,
          year: this.preferences[i].year,
          date: this.preferences[i].date,
          competitionId: this.preferences[i].competitionId,
          preferenceId: this.preferences[i].preferenceId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-preferences',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPreferencesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.preferenceToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.preferenceToAdd);
      console.log(jsonData);
      
      const res = await PreferenceService.addPreference(jsonData);
