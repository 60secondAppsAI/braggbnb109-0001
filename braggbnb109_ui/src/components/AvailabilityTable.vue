        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Availabilitys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalAvailabilitys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAvailabilitys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Availability</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AvailabilityId" type="text" placeholder="Enter AvailabilityId" v-model="availabilityToAdd.availabilityId"></base-input>
  <base-input label="AvailableDate" type="text" placeholder="Enter AvailableDate" v-model="availabilityToAdd.availableDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="availabilitys" :row-key="record => record.AvailabilityId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AvailabilityPictureView :availabilitys="availabilitys" />
          </div>
...
import AvailabilityPictureView from './AvailabilityPictureView.vue';


const availabilitysColumns = [
  "availabilityId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    AvailabilityPictureView
  },

  data() {
    return {
      modalAvailabilitys: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'availabilityId',
          visible: true,
          scopedSlots: { customRender: 'availabilityId' },
          sorter: true
          //sorter: (a, b) => a.availabilityId - b.availabilityId,
          //sorter: (a, b) => a.availabilityId.localeCompare(b.availabilityId),
        },
        {
          title: 'Available Date',
		dataIndex: 'availableDate',
          visible: true,
          scopedSlots: { customRender: 'availableDate' },
          sorter: true
          //sorter: (a, b) => a.availableDate - b.availableDate,
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} availabilitys`,
      },
      ...
      availabilitysTable: {
        columns: [...availabilitysColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'availabilityId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAvailabilitysTable() {
      ...
      let availabilitysTableData = [];
      for (let i = 0; i < this.availabilitys.length; i++) {
        availabilitysTableData.push({
          id: i,
          availabilityId: this.availabilitys[i].availabilityId,
          year: this.availabilitys[i].year,
          date: this.availabilitys[i].date,
          competitionId: this.availabilitys[i].competitionId,
          availabilityId: this.availabilitys[i].availabilityId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-availabilitys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAvailabilitysChanged', this.searchQuery);
		//this.renderAvailabilitysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalAvailabilitys = false;

      const currentDate = new Date().getTime();
      this.availabilityToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.availabilityToAdd);
      console.log(jsonData);
      
      const res = await AvailabilityService.addAvailability(jsonData);
