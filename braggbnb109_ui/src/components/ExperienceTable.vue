...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalExperiences = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalExperiences">
                <template slot="header">
  ...
  <base-input label="Title" type="text" placeholder="Enter Title" v-model="experienceToAdd.title"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="experienceToAdd.description"></base-input>
  <base-input label="Price" type="text" placeholder="Enter Price" v-model="experienceToAdd.price"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="experiences" :row-key="record => record.ExperienceId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ExperiencePictureView :experiences="experiences" />
          </div>
...
import ExperiencePictureView from './ExperiencePictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    ExperiencePictureView
  },

  data() {
    return {
      modalExperiences: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'experienceId',
          visible: true,
          scopedSlots: { customRender: 'experienceId' },
          sorter: true
          //sorter: (a, b) => a.experienceId - b.experienceId,
          //sorter: (a, b) => a.experienceId.localeCompare(b.experienceId),
        },
        {
          title: 'Title',
		dataIndex: 'title',
      ...
      sortBy: 'experienceId',           // Column to sort by
  ...
  methods: {
    ...
    async renderExperiencesTable() {
      ...
      let experiencesTableData = [];
      for (let i = 0; i < this.experiences.length; i++) {
        experiencesTableData.push({
          id: i,
          experienceId: this.experiences[i].experienceId,
          year: this.experiences[i].year,
          date: this.experiences[i].date,
          competitionId: this.experiences[i].competitionId,
          experienceId: this.experiences[i].experienceId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-experiences',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForExperiencesChanged', this.searchQuery);
		//this.renderExperiencesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalExperiences = false;

      const currentDate = new Date().getTime();
      this.experienceToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.experienceToAdd);
      console.log(jsonData);
      
      const res = await ExperienceService.addExperience(jsonData);
