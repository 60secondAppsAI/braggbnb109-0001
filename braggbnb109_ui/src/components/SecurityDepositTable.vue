        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Security Deposits</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalSecurityDeposits = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalSecurityDeposits">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add SecurityDeposit</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="SecurityDepositId" type="text" placeholder="Enter SecurityDepositId" v-model="securityDepositToAdd.securityDepositId"></base-input>
  <base-input label="Amount" type="text" placeholder="Enter Amount" v-model="securityDepositToAdd.amount"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="securityDeposits" :row-key="record => record.SecurityDepositId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <SecurityDepositPictureView :securityDeposits="securityDeposits" />
          </div>
...
import SecurityDepositPictureView from './SecurityDepositPictureView.vue';


const securityDepositsColumns = [
  "securityDepositId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    SecurityDepositPictureView
  },

  data() {
    return {
      modalSecurityDeposits: false,
        isTableView: true,

      columns: [
        {
          title: 'Security Deposit Id',
		dataIndex: 'securityDepositId',
          visible: true,
          scopedSlots: { customRender: 'securityDepositId' },
          sorter: true
          //sorter: (a, b) => a.securityDepositId - b.securityDepositId,
          //sorter: (a, b) => a.securityDepositId.localeCompare(b.securityDepositId),
        },
        {
          title: 'Amount',
		dataIndex: 'amount',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} securityDeposits`,
      },

      securityDeposits: [],
      securityDepositToAdd: {},

      securityDepositsTable: {
        columns: [...securityDepositsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'securityDepositId',           // Column to sort by
  ...
  methods: {
    ...
    async renderSecurityDepositsTable() {
      ...
      let securityDepositsTableData = [];
      for (let i = 0; i < this.securityDeposits.length; i++) {
        securityDepositsTableData.push({
          id: i,
          securityDepositId: this.securityDeposits[i].securityDepositId,
          year: this.securityDeposits[i].year,
          date: this.securityDeposits[i].date,
          competitionId: this.securityDeposits[i].competitionId,
          securityDepositId: this.securityDeposits[i].securityDepositId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-security-deposits',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForSecurityDepositsChanged', this.searchQuery);
		//this.renderSecurityDepositsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalSecurityDeposits = false;

      const currentDate = new Date().getTime();
      this.securityDepositToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.securityDepositToAdd);
      console.log(jsonData);
      
      const res = await SecurityDepositService.addSecurityDeposit(jsonData);
