        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalPropertyTypes = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPropertyTypes">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add PropertyType</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PropertyTypeId" type="text" placeholder="Enter PropertyTypeId" v-model="propertyTypeToAdd.propertyTypeId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="propertyTypeToAdd.name"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="propertyTypeToAdd.description"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="propertyTypes" :row-key="record => record.PropertyTypeId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PropertyTypePictureView :propertyTypes="propertyTypes" />
          </div>
...
import PropertyTypePictureView from './PropertyTypePictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    PropertyTypePictureView
  },

  data() {
    return {
      modalPropertyTypes: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'propertyTypeId',
          ...
          sorter: true
          //sorter: (a, b) => a.propertyTypeId - b.propertyTypeId,
          //sorter: (a, b) => a.propertyTypeId.localeCompare(b.propertyTypeId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} propertyTypes`,
      },
      ...
      sortBy: 'propertyTypeId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPropertyTypesTable() {
      ...
      let propertyTypesTableData = [];
      for (let i = 0; i < this.propertyTypes.length; i++) {
        propertyTypesTableData.push({
          id: i,
          propertyTypeId: this.propertyTypes[i].propertyTypeId,
          year: this.propertyTypes[i].year,
          date: this.propertyTypes[i].date,
          competitionId: this.propertyTypes[i].competitionId,
          propertyTypeId: this.propertyTypes[i].propertyTypeId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-property-types',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPropertyTypesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalPropertyTypes = false;

      const currentDate = new Date().getTime();
      this.propertyTypeToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.propertyTypeToAdd);
      console.log(jsonData);
      
      const res = await PropertyTypeService.addPropertyType(jsonData);
